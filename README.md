...
本人の文脈（筋トレ・体づくりの日常）を題材にした**淡々トーンの日記**＋**軽セクシー画像**＋
**Patreon/X 誘導カード**で投稿します。※ギャル口調は使わない（普通の一人称で淡々と）。
記事本文は `everyday_content.py` のテーマ（トレ後／気づき／ジム／食事／回復／日常）から
毎回違う組み合わせで作ります。テーマ追加は同ファイルの `THEMES` に追記。人物実名・数字は書かない。
